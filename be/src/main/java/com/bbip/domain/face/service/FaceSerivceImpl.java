...
import com.bbip.global.exception.FaceNotFoundException;
...
public class FaceSerivceImpl implements FaceService {
    ...
    public FaceDto addFace(String accessToken, FaceDto face, MultipartFile image) {
        ...
        // 인앱에서 사진 찍을때 임베딩값을 뽑아서 보내줘야 할 듯
...
//        face.setFaceEmbedding(faceembedding);
    ...
    @Override
    public void removeFace(String accessToken, Integer id) {

        // JWT토큰에서 사용자 id 추출
        int userId = jwtUtil.getUserIdFromJWT(accessToken);

        String fileName = faceRepository.findFileNameById(userId, id);
        if (fileName == null) throw new FaceNotFoundException("해당 계정에 등록된 얼굴이 아닙니다.");
